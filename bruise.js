...
window.loadModel = async function () {
  ...
  let detectionModel, segmentationModel, bruiseModel;
  // 加载蓝莓检测模型
  let start1 = (new Date()).getTime()
  detectionModel = new onnx.InferenceSession();
  await detectionModel.loadModel('./blueberry_detection_model.onnx');
  let end1 = (new Date()).getTime()
  message('message1',`ImageCrop_model loaded in ${(end1 - start1) / 1000} secs`, true)
  
  // 加载果肉分割模型
  let start2 = (new Date()).getTime()
  segmentationModel = new onnx.InferenceSession();
  await segmentationModel.loadModel('./blueberry_segmentation_model.onnx');
  let end2 = (new Date()).getTime()
  message('message1',`berrySeg_model loaded in ${(end2 - start2) / 1000} secs`, true)
  ...
  // 加载瘀伤模型
  let start3 = (new Date()).getTime()
  bruiseModel = new onnx.InferenceSession();
  await bruiseModel.loadModel('./blueberry_bruise_model.onnx');
  let end3 = (new Date()).getTime()
  message('message1',`bruiseSeg_model loaded in ${(end3 - start3) / 1000} secs`, true)
  // 激活“选择文件”按钮
  document.getElementById('modelimage').disabled = false
